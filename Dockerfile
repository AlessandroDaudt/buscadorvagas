...
RUN pip install --upgrade pip \
    && pip install -c constraints.lock ".[mcp,documents]"

FROM python:3.12-slim AS runtime
LABEL org.opencontainers.image.source="https://github.com/tarunlnmiit/autopilot-jobhunt" \
      org.opencontainers.image.description="Local-only, user-reviewed job discovery agent" \
...
COPY alembic.ini ./
COPY migrations ./migrations
RUN mkdir -p state output config resume \
